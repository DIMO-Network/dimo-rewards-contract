...
contract DIMORegistry is ERC721, AccessControlInternal {
  ...
  event ModuleUpdated(address indexed oldImplementation, address indexed newImplementation, bytes4[] oldSelectors, bytes4[] newSelectors);
  ...
  fallback() external {
    address implementation = DIMOStorage.getStorage().implementations[msg.sig];
    assembly {
  ...
  function updateModule(
    ...
    emit ModuleUpdated(oldImplementation, newImplementation, oldSelectors, newSelectors);
  }
  ...
  function addModule(address implementation, bytes4[] calldata selectors) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function removeModule(address implementation, bytes4[] calldata selectors) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /// @param implementation address of the implementation
  /// @param selectors selectors of the implementation contract
  function _addModule(address implementation, bytes4[] calldata selectors) private {
    DIMOStorage.Storage storage s = DIMOStorage.getStorage();
    require(s.selectorsHash[implementation] == 0x0, "Implementation already exists");

    for (uint256 i = 0; i < selectors.length; i++) {
      require(s.implementations[selectors[i]] == address(0), "Selector already registered");
  ...
  /// @param implementation implementation address
  /// @param selectors function signatures
  function _removeModule(address implementation, bytes4[] calldata selectors) private {
    DIMOStorage.Storage storage s = DIMOStorage.getStorage();
    bytes32 hash = keccak256(abi.encode(selectors));
    require(s.selectorsHash[implementation] == hash, "Invalid selector list");

    for (uint256 i = 0; i < selectors.length; i++) {
      require(s.implementations[selectors[i]] == implementation, "Unregistered selector");
